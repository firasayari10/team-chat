...
export const Thread = ({ messageId, onClose }: ThreadProps) => {
  ...
  const groupedMessages = results?.reduce((groups, message) => {
    const dateKey = new Date(message._creationTime).toISOString().split("T")[0];
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="px-4">
        <Editor key={editorKey} onSubmit={handleSubmit} disabled={isPending} innerRef={editorRef} placeholder="Reply ..." />
      </div>
    </div>
  );
};
