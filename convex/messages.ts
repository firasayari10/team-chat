...
const populateThread = async (ctx:QueryCtx , messageId: Id<"messages">)=> {
    ...
    return {
        ...
        timestam: lastMessage._creationTime
            ...
            page: (
                await Promise.all(
                    results.page.map(async(message)=>
                    {
                        ...
                        const reactionsWithoutMemberIdProprety = dedupedReactions.map(
                            ({memberId , ...rest}) => rest ,
                        );
                        return {
                            ...message,
                            ...
                            member,
                            user,
                            reactions:reactionsWithoutMemberIdProprety,
...
export const create = mutation ({
    ...
    handler: async (ctx , args)=> {
        ...
        if(!args.conversationId && !args.channelId && args.parentMessageId)
        {
            ...
            if(!parentMessage)
            {
                throw new Error (" Parent message missing !")
            }
            _conversationId = parentMessage.conversationId;
