  ...
  args:{
    ...
    handler:async (ctx,args)=>{
      ...
      if(currentMember._id === args.id && currentMember.role === "admin") 
        ...
        const [mesages , reactions , conversations] = await Promise.all([
          ctx.db.query("messages")
          .withIndex("by_member_id", (q)=> q.eq("memberId",member._id)).collect(),
          ctx.db.query("reactions")
          .withIndex("by_memnber_id", (q)=> q.eq("memberId",member._id)).collect(),
